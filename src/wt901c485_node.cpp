...
void serialCallback(int32_t signal_){
	...
	if(serial.recv_data[0] == 0x50 and res == serial.recv_data[2]+5 and serial.recv_data[1] == 0x03){			//check data length
		...
		imu_data.angular_velocity.x    = static_cast<int16_t>((serial.recv_data[9] << 8) | serial.recv_data[10]) / 32768.0f * gyr_range * M_PI / 180.0f;
		imu_data.angular_velocity.y    = static_cast<int16_t>((serial.recv_data[11] << 8) | serial.recv_data[12]) / 32768.0f * gyr_range * M_PI / 180.0f;
		imu_data.angular_velocity.z    = static_cast<int16_t>((serial.recv_data[13] << 8) | serial.recv_data[14]) / 32768.0f * gyr_range * M_PI / 180.0f;
